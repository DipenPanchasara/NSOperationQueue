...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    [cvImages setDelegate:self];
    [cvImages setDataSource:self];
...
{
    DemoCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"DemoCell"
    ...
    if([array count] > 0)
    {
        ...
        {
            if (cvImages.dragging == NO && cvImages.decelerating == NO)
            {
                [self downloadImageForURL:[NSURL URLWithString:array[indexPath.row]] AtIndexPath:indexPath];
            }
            cell.imgViewIcon.image = [UIImage imageNamed:@"Placeholder.png"];
        }
        else
        {
            [cell.imgViewIcon setImage:[UIImage imageWithContentsOfFile:filePath]];
        }
    }
    ...
    if(![dictOperations objectForKey:indexPath])
    {
        ...
        [obj setSuccessHandler:^(id result) {
            dispatch_sync(dispatch_get_main_queue(), ^{
                DemoCell *cell = (DemoCell *)[cvImages cellForItemAtIndexPath:indexPath];
                cell.imgViewIcon.image = result;
            });
...
- (void)loadImagesForOnscreenRows
{
    if ([array count] > 0)
    {
        NSArray *visiblePaths = [cvImages indexPathsForVisibleItems];
